...
@test "init fails when no aws credentials are set" {
...
@test "init creates a valid config file without a provider" {
  file=$(fixture encrypted.kms)
  rm "$file"
...
@test "init creates a valid config file with a provider" {
  file=$(fixture encrypted.kms)
  rm "$file"
...
@test "init creates a valid config file with arguments" {
  file=$(fixture encrypted.kms)
  rm "$file"
